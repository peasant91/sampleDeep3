...
export const LatoSemiBold = ({children, style, numOfLines, Icon, containerStyle, iconTint}) => {
  return (
      <View style={[{flexDirection: 'row', alignItems: 'center'}, containerStyle]}>
        {Icon ? <Icon style={{paddingRight: 10, color: iconTint}}/> : null}
      <Text ellipsizeMode='tail' numberOfLines={numOfLines} style={[{fontFamily: 'Lato-SemiBold', color: Colors.primaryText, fontSize: 14, paddingLeft: Icon ? 5 : 0}, style]}>{children}</Text>
      </View>
  );
};
