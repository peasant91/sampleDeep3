...
const AccountScreen = ({navigation, route}) => {
  ...
  const goToEdit = () => {
    ...
    if (profileData && (Object.keys(profileData).length != 0)) {
      navigation.navigate('EditProfile', {
        isEdit: true,
        data: profileData,
        isVerified: profileData.status == 'verified' ? true : false,
      });
    }
  };
  ...
  const goToContract = () => {
    navigation.navigate('CurrentContract', {
      id: contractData?.active_contract?.contract_id,
      isEmpty: contractData.active_contract == null,
      isCurrent: true,
    });
  };
    ...
    navigation.navigate('Bank', {
      isReadOnly: profileData.account_bank
        ? profileData.status === 'verified'
          ...
          : false
        : true,
      prevData: profileData.account_bank
        ? profileData.account_bank
        : profileData.driver_company.account_bank,
    });
  };
