...
const CurrentContractScreen = ({navigation, route}) => {
    ...
    const goToCrudReport = (isAdd, id) => {
        navigation.navigate('CrudReport', {
            isAdd: isAdd,
            id: id,
            stickerArea: contractData.campaign.sticker_area,
        });
    };
                ...
                axios
                    ...
                    .then(
                        axios.spread(async (report, contract, chart, example) => {
                            console.log("widireport", report)
                            setcontractData(contract);
                            console.log("widcontract", contract)
    ...
    const handleCreateReport = () => {
        ...
        if (reportData.length > 0) {
            ...
            if (!firstReport.is_driver) {
              ...
              showDialog(
                translate('report_warning_title'),
                false,
                () => {},
                null,
                translate('back'),
                null,
                null,
                translate('report_warning_desc'),
              );
            }
        } else {
            //show Dialog
            showDialog(
                translate('report_invalid_title'),
                false,
                () => {
                },
                null,
        ...
        <>
            <SafeAreaView style={{flex: 1, backgroundColor: 'white'}}>
                ...
                {!isEmpty ? (
                        ...
                        <ScrollView style={{flex: 1}}>
                            <View style={{flex: 1, marginBottom: 16}}>
                                <View style={{flexDirection: 'row', padding: 16}}>
                                    <Image
                                        source={{
                                            uri: getFullLink(contractData?.campaign.company_image),
                                        }}
                                    ...
                                    <View style={{paddingLeft: 16, flex: 1}}>
                                        <LatoBold style={{color: '#6D6D6D'}}>
                                            {contractData.campaign.company_name}
                                        </LatoBold>
                                        <LatoRegular
                                            style={{
                                            ...
                                            Icon={IconLocation}>
                                            {displayProvince(contractData.campaign.contract_area)}
                                        </LatoRegular>
                                        <LatoRegular style={{fontSize: 10, color: '#A7A7A7'}}>
                                            {translate('from_', {date: getAdsDate()})}
                                        </LatoRegular>
                                        <StatusTag
                                            text={contractData.campaign.status}
                                    ...
                                    <TouchableOpacity
                                        onPress={() =>
                                            navigation.navigate('InstallationList', {
                                                data: contractData.campaign.installation_schedule,
                                            })
                                        }>
                                        <LatoBold
                                            style={{
                                ...
                                {contractData.campaign.installation_schedule &&
                                    contractData.campaign.installation_schedule.map(
                                        (item, index) => {
                                            return (
                                ...
                                {contractData.campaign.status == 'Selesai' && (
                                    <>
                                        <View style={{padding: 16}}>
                                            <KeyValueComponent
                                                title={translate('profit')}
                                                value={toCurrency(contractData.total_profit)}
                                                isBold
                                                style={styles.subHeading}
                                            />
                                            <KeyValueComponent
                                                title={translate('price_per_kilos')}
                                                value={toCurrency(contractData.campaign.price_km)}
                                                style={styles.subHeading}
                                            />
                                        </View>
                                        <Divider/>
                                        <View style={{padding: 16}}>
                                            ...
                                            <KeyValueComponent
                                                ...
                                                value={contractData.bank_number}
                                                style={styles.subHeading}
                                            />
                                        </View>
                                        <Divider/>
                                ...
                                <View style={{padding: 16}}>
                                    <LatoBold Icon={IconReport} style={{color: Colors.primary}}>
                                        {translate('trip_report')}
                                    </LatoBold>
                                    <KeyValueComponent
                                        title={translate('driver_status')}
                                        value={contractData.status}
                                        style={styles.subHeading}
                                        containerStyle={{marginTop: 10}}
                                    />
                                    <KeyValueComponent
                                        title={translate('vehicle_type')}
                                        value={contractData.vehicle_type}
                                        style={styles.subHeading}
                                    />
                                    <KeyValueComponent
                                        title={translate('ads_type')}
                                        value={
                                            contractData.campaign.sticker_area.length == 1
                                                ? contractData.campaign.sticker_area[0].name
                                                : contractData.campaign.sticker_area
                                                    .map(item => item.name)
                                                    .join(', ')
                                        }
                                        style={styles.subHeading}
                                    />
                                    <KeyValueComponent
                                        title={translate('total_trip')}
                                        value={contractData.total_distance + 'Km'}
                                        style={styles.subHeading}
                                    />
                                    <KeyValueComponent
                                        title={translate('trip_today')}
                                        value={contractData.today_distance + 'Km'}
                                        style={styles.subHeading}
                                    />
                                </View>
                    ...
                    <EmptyContract
                        desc={translate('no_active_contract')}
                        onPress={seeOffer}
                    />
                )}
            </SafeAreaView>
